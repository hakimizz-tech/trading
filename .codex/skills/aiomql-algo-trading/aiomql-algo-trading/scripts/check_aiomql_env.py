...
def check_project(project: Path) -> int:
    ...
    expected = ["pyproject.toml", "requirements.txt", "uv.lock", "aiomql.json", "aiomql.json.example", ".gitignore", "strategies"]
    ...
    if config_path.exists():
        ...
        print("aiomql.json: not found. This is OK for templates, but live/demo MT5 login needs a local file or programmatic Config.")

    gitignore = project / ".gitignore"
    if gitignore.exists():
        text = gitignore.read_text(encoding="utf-8", errors="ignore")
        for needed in ["aiomql.json", ".env", "logs/", "trade_results/"]:
            status = "OK" if needed in text else "WARN"
            print(f".gitignore contains {needed}: {status}")
    else:
        print(".gitignore: WARN, create one before adding credentials or results")

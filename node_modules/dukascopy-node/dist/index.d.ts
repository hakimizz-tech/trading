import { ValidationRule, ValidationError } from 'fastest-validator';
export { RuleBoolean, RuleDate, RuleNumber, RuleObject, RuleString } from 'fastest-validator';
...
type PriceType = keyof typeof Price;
...
type TimeframeType = keyof typeof Timeframe;
...
type VolumeUnitType = keyof typeof VolumeUnit;
...
type DefaultConfig = Required<Pick<ConfigBase, OptionalKeys<ConfigBase>>>;
declare const defaultConfig: DefaultConfig;
interface ConfigArrayTickItem extends ConfigBase {
    timeframe?: 'tick';
    format?: 'array';
}
interface ConfigArrayItem extends ConfigBase {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'array';
}
interface ConfigJsonTickItem extends ConfigBase {
    timeframe?: 'tick';
    format?: 'json';
}
interface ConfigJsonItem extends ConfigBase {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'json';
}
interface ConfigCsvItem extends ConfigBase {
    format?: 'csv';
}
type Config = ConfigArrayTickItem | ConfigArrayItem | ConfigJsonTickItem | ConfigJsonItem | ConfigCsvItem;

type InputSchema<Input> = Record<keyof Required<Input>, ValidationRule>;
declare const schema: InputSchema<Config>;

type ConfigValidationError = Pick<ValidationError, 'message' | 'expected' | 'actual'>;
type SanitizedConfig<InputConfig extends ConfigBase> = Required<Omit<InputConfig, 'dates'>> & {
    dates: {
        from: Date;
        to: Date;
    };
};
interface ValidateConfigOutput<InputConfig extends ConfigBase> {
 ...
 *
 */
declare function validateConfig<InputConfig extends ConfigBase>(input: InputConfig, schema: InputSchema<InputConfig>): ValidateConfigOutput<InputConfig>;
declare function validateConfigNode(input: Config): ValidateConfigOutput<Config>;

interface NormaliseDatesInput {
    instrument: InstrumentType;
    startDate: Date;
    endDate: Date;
    timeframe: TimeframeType;
...
type CacheKeyFormatter = (input: string) => string;
interface CacheManagerBase {
...
declare class CacheManager implements CacheManagerBase {
    ...
    readItemFromCache(key: string): Promise<Buffer | null>;
    writeItemsToCache(items: BufferObject[]): Promise<PromiseSettledResult<void>[]>;
    purgeCache(cacheFolderPath?: string): Promise<void>;
    getCacheKeyFromUrl(key: string): string;
}

declare class BufferFetcher {
    batchSize: number;
    pauseBetweenBatchesMs: number;
    onItemFetch?: BufferFetcherInput['onItemFetch'];
    onBatchFetch?: BufferFetcherInput['onBatchFetch'];
    fetcherFn: ((url: string) => Promise<Buffer>) | undefined;
    retryCount: number;
    retryOnEmpty: boolean;
    pauseBetweenRetriesMs: number;
    failAfterRetryCount: BufferFetcherInput['failAfterRetryCount'];
    cacheManager?: CacheManagerBase;
    constructor({ batchSize, pauseBetweenBatchesMs, onItemFetch, onBatchFetch, fetcherFn, retryCount, retryOnEmpty, failAfterRetryCount, pauseBetweenRetriesMs, cacheManager }: BufferFetcherInput);
    private fetchBatch;
    /**
     * @experimental
     */
    fetch_optimized(urls: string[]): Promise<true>;
...
type ProcessDataOutput = number[][];

declare function processData({ instrument, requestedTimeframe, bufferObjects, priceType, volumes, volumeUnits, ignoreFlats }: ProcessDataInput): ProcessDataOutput;
...
type Output = JsonItem[] | JsonItemTick[] | string | ArrayItem[] | ArrayTickItem[];

declare function formatOutput({ processedData, timeframe, format }: {
...
declare function getHistoricalRates(config: ConfigArrayItem): Promise<ArrayItem[]>;
declare function getHistoricalRates(config: ConfigArrayTickItem): Promise<ArrayTickItem[]>;
declare function getHistoricalRates(config: ConfigJsonItem): Promise<JsonItem[]>;
declare function getHistoricalRates(config: ConfigJsonTickItem): Promise<JsonItemTick[]>;
declare function getHistoricalRates(config: ConfigCsvItem): Promise<string>;
declare function getHistoricalRates(config: Config): Promise<Output>;
/**
...
declare const instrumentMetaData: Record<InstrumentType, InstrumentMetaData>;

export { ArrayItem, ArrayTickItem, BufferFetcher, CacheManager, Config, ConfigArrayItem, ConfigArrayTickItem, ConfigCsvItem, ConfigJsonItem, ConfigJsonTickItem, ConfigValidationError, DefaultConfig, Format, FormatType, InputSchema, Instrument, InstrumentType, JsonItem, JsonItemTick, Output, Price, PriceType, RealTimeRatesConfig, RealTimeRatesConfigArrayItem, RealTimeRatesConfigArrayTickItem, RealTimeRatesConfigCsv, RealTimeRatesConfigJsonItem, RealTimeRatesConfigJsonTickItem, SanitizedConfig, Timeframe, TimeframeType, URL_ROOT, ValidateConfigOutput, defaultConfig, formatOutput, generateUrls, getCurrentRates, getHistoricRates, getHistoricalRates, getRealTimeRates, instrumentMetaData, normaliseDates, processData, schema, validateConfig, validateConfigNode };

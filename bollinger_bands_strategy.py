...
def generate_adaptive_bollinger_signals(
    ...
    mean_reversion_long = wide_bandwidth & lower_cross_up & (rsi < cfg.rsi_oversold)
    mean_reversion_short = wide_bandwidth & upper_cross_down & (rsi > cfg.rsi_overbought)
    breakout_long = squeeze_recent & breakout_upper & macd_cross_up & volume_ok
    breakout_short = squeeze_recent & breakout_lower & macd_cross_down & volume_ok
    ...
    result["long_exit"] = middle_cross_up.fillna(False) | macd_cross_down.fillna(False)
    result["short_exit"] = middle_cross_down.fillna(False) | macd_cross_up.fillna(False)
...
def run_strategy(
    ...
    if strategy == "adaptive":
        ...
        raise ValueError("strategy must be one of: mean_reversion, bbma, bb_rsi, adaptive")
